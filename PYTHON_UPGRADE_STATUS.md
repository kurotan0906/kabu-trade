...
**重要**: このエラーはmacOSのセキュリティ設定（フルディスクアクセス）が原因の可能性が高いです。

## セキュリティ設定の確認（必須）

仮想環境を作成する前に、以下の設定を確認してください：
...
3. **フルディスクアクセスを確認**
   - 左側の「フルディスクアクセス」を選択
   - 使用中のターミナルアプリ（Terminal.app または iTerm2）にチェックが入っているか確認
   - チェックがない場合は、ロックを解除して追加

4. **ターミナルを再起動**
   - 設定変更後はターミナルを完全に再起動してください
